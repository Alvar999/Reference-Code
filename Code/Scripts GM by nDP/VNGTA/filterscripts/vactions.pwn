...
CMD:ngoighe(playerid, params[])
{
	...
	switch(strval(params))
	{
	case 1: PlayAnimEx(playerid,"INT_OFFICE", "OFF_Sit_Bored_Loop", 1.800001, 1, 0, 0, 1, 600, 0);
	case 2: PlayAnimEx(playerid,"CRIB","PED_Console_Loop",4.0,1,0,0,0,0, 0);
	case 3: PlayAnimEx(playerid,"INT_HOUSE","LOU_In",4.0,0,0,0,1,0, 0);
	case 4: PlayAnimEx(playerid,"MISC","SEAT_LR",4.0,1,0,0,0,0, 0);
	case 5: PlayAnimEx(playerid,"MISC","Seat_talk_01",4.0,1,0,0,0,0, 0);
	case 6: PlayAnimEx(playerid,"MISC","Seat_talk_02",4.0,1,0,0,0,0, 0);
	case 7: PlayAnimEx(playerid,"ped","SEAT_down",4.0,0,0,0,1,0, 0);
	case 8: PlayAnimEx(playerid,"BASEBALL","Bat_4",4.1,1,0,0,1,11000, 0);
	default: SendClientMessage(playerid, COLOR_WHITE, "USAGE: /ngoighe [1-8]");
	}
	return 1;
}
...
CMD:fallover(playerid, params[])
{
	...
	case 6: PlayAnimEx(playerid, "PED","FLOOR_hit_f", 4.0, 1, 0, 0, 0, 0, 1);
...
{
    lapdtext = CreateObject(19464, 1554.94, -1675.62, 22.96,   0.00, 0.00, 0.00);
    SetObjectMaterialText(lapdtext, "{0000FF}Los Angeles\nPolice Department", 0, OBJECT_MATERIAL_SIZE_256x128, "Arial", 35, 1, 0, 0, OBJECT_MATERIAL_TEXT_ALIGN_CENTER);
    doctext = CreateObject(19464, 1901.61, -1740.40, 15.89,   0.00, 0.00, 89.50);
    SetObjectMaterialText(doctext, "{0000FF}Department Of Corrections", 0, OBJECT_MATERIAL_SIZE_256x128, "Arial", 20, 1, 0, 0, OBJECT_MATERIAL_TEXT_ALIGN_CENTER);
    return 1;
}
CMD:testb(playerid, params[])
{
    DestroyObject(lapdtext);
    DestroyObject(doctext);
    return 1;
}

CMD:xmask(playerid, params[])
{
    for(new i = 0; i < MAX_PLAYERS; i++) ShowPlayerNameTagForPlayer(i, playerid, false);
    GameTextForPlayer(playerid, "~W~Nametags ~R~off", 5000, 5);
}

CMD:xmaskoff(playerid, params[])
{
